...
// 构造 / 重置
// ===============================
WorkspaceManager::WorkspaceManager(AIController& aiCtrl)
    : ai(aiCtrl)
{
    reset();
}
...
// 构建 Workspace
// ===============================
bool WorkspaceManager::buildFromUserInput(const std::string& userInput)
{
    error_message.clear();
    ai_output.clear();
...
// 状态判断
// ===============================
bool WorkspaceManager::isReadyForDecision() const
{
    if (!hasWorkspace()) return false;
    return workspace_json.find("\"decision_model\"") != std::string::npos;
}

bool WorkspaceManager::isReadyForPLC() const
{
    if (!hasWorkspace()) return false;
...
// Getter
// ===============================
const std::string& WorkspaceManager::getUserInput() const
{
    return user_input;
}

const std::string& WorkspaceManager::getAiRawOutput() const
{
    return ai_output;
}

const std::string& WorkspaceManager::getWorkspaceJson() const
{
    return workspace_json;
}

const std::string& WorkspaceManager::getErrorMessage() const
{
...
// 文件操作
// ===============================
bool WorkspaceManager::saveToFile(const std::string& path) const
{
    if (!hasWorkspace()) return false;
...
bool WorkspaceManager::loadFromFile(const std::string& path)
{
...
// 调用 AI
// ===============================
bool WorkspaceManager::callWorkspaceAI(const std::string& userInput)
{
    std::string prompt;

    if (hasWorkspace())
    {
        prompt += u8"CURRENT_WORKSPACE_JSON:\n";
        prompt += workspace_json;
        prompt += u8"\n\nUSER_UPDATE:\n";
        prompt += userInput;
    }
    else
    ...
    AIController::WorkspaceDraft draft = ai.createWorkspace(prompt);

    if (!draft.ok)
    {
        error_message = draft.err.empty()
            ? u8"AI 调用失败"
            : draft.err;
        return false;
    }

    ai_output = draft.raw_json;
    if (ai_output.empty())
    {
...
// JSON 提取
// ===============================
bool WorkspaceManager::extractWorkspaceJson(const std::string& aiText)
{
    workspace_json.clear();
...
// JSON 括号提取
// ===============================
bool WorkspaceManager::extractFirstJsonObject(
    const std::string& s,
    size_t startPos,
    std::string& outJson)
{
